...
(async()=>{
 ...
 assert.equal(await page.locator('.nav-item').count(),10);assert.equal(await page.locator('.flow-row').count(),6);
 ...
 for(const id of ['burn','poison','shield','energy','ammo','critical','status','tempo','healing','frost']){
  await page.locator(`[data-mechanism="${id}"]`).click();
  assert.equal(await page.locator('.flow-row').count(),await page.evaluate(id=>window.ATLAS_DATA.mechanisms.find(m=>m.id===id).rules.length,id));
  // Every rendered statement is checked against the source-loaded rule, not an invented summary.
  assert.ok(await page.evaluate(id=>{const m=window.ATLAS_DATA.mechanisms.find(m=>m.id===id);return m.rules.every(r=>{const row=document.querySelector(`[data-rule="${r.id}"]`),item=row.querySelector('.flow-item strong');return row.querySelector('.result-text').textContent===r.result&&(!r.itemId||(item&&item.textContent===r.item))})},id));
 }
